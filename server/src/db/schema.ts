...
export const learningProgressDevices = mysqlTable(
  'learning_progress_devices',
  {
    ...
    maxProgress: decimal('max_progress', { precision: 5, scale: 4, mode: 'number' })
      .notNull()
      .default(0),
    resumeHeadingId: varchar('resume_heading_id', { length: 255 }),
    resumeProgress: decimal('resume_progress', { precision: 5, scale: 4, mode: 'number' }),
  ...
  (table) => [
    ...
    check('learning_progress_max_progress_ck', sql`${table.maxProgress} between 0 and 1`),
    check(
      'learning_progress_resume_progress_ck',
      sql`${table.resumeProgress} is null or ${table.resumeProgress} between 0 and 1`,
    ),
  ],
);
